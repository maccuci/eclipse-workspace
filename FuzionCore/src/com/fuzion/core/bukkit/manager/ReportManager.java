...
public class ReportManager {
	...
	public static void addReport(Player reporter, Player reported, String reason) {
	...
	public static void sendReports(Player player) {
		player.sendMessage("§aListando todos os reports.");
		player.sendMessage(" ");
		for(Report report : reports) {
			player.sendMessage("Reporter: §b" + report.getReporter().getName());
			player.sendMessage("Reportado: §b" + report.getReported().getName());
			player.sendMessage("Motivo: §b" + report.getReason());
			player.sendMessage(" ");
			player.sendMessage("§aServidor: §f" + report.getReported().getServer().getIp());
		}
	}
	...
	public static void menu(Player player, int page) {
		...
		for(int i = 0; i < reportsList.size(); i++) {
			...
			PunishManager punishManager = new PunishManager(null, null, report.getReported(), null, 0);
			...
			items.add(new MenuItem(new ItemBuilder().type(Material.SKULL_ITEM).durability(3).skin(report.getReported().getName()).name("§b" + report.getReported().getName()).lore("§fReportado por: §e" + report.getReporter().getName(), "§fData: §b" + report.getDate(), "§fMotivo: §b" + report.getReason(), " ", "§3§lDIREITO §fpara recusar.", "§3§lESQUERDO §fpara aceitar.").build(), new MenuClickHandler() {
				...
				public void onClick(Player paramPlayer, Inventory paramInventory, ClickType paramClickType, ItemStack paramItemStack, int paramInt) {
					if(paramClickType == ClickType.LEFT) {
						...
						paramPlayer.closeInventory();
						paramPlayer.teleport(report.getReported());
						...
						report.getReporter().sendMessage("§cSeu report foi negado!");
						reports.remove(report);
